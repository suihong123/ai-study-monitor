...
> 基线日期：2026-08-06
> 生产代码：`68ed8d4`（v0.9）
> 实施状态：v0.9 已完成生产迁移、部署和最终真机烟雾验收；v0.9.1 每日只读保活已通过 Preview 验证并获 Production 发布确认
...
- 使用访问码而非用户账号进入；访问码具备套餐、总时长、状态和单一使用环境激活。
...
- v0.9 已将设备绑定修正为浏览器“使用环境”激活：默认任意连续 15 天最多 10 次免费重新激活、两次成功至少间隔 60 秒，不扣监督时长；该能力已完成生产验收。
- v0.9 测试代码在监督结束、重新绑定、卸载和返回首页时取消分析/心跳等请求，并以前端会话版本和数据库令牌事务复核共同隔离旧响应；该加固不改变重新绑定、分钟、会话或报告规则。
- Preview 已验证真实 Qwen 调用，核心真机场景已确认同环境恢复、Mac→手机重新绑定、再次进入稳定及监督中被新环境接管。
...
- 已确认正式线上环境为 `www.aistudyguard.top`；实际家庭数、付费客户和稳定流量仍未知。
- 访问码从何处售卖、实际售价、退款率、获客渠道和客服流程。
- 真实识别准确率、设备兼容率、监督完成率、留存和续费。
- `migration_2026_17` 已在生产 Supabase 执行并验证；更早迁移是否全部有独立记录、当前启用 Mock 还是 Qwen 仍待核对。
...
当前没有账户体系，因此一个访问码基本承担了“客户身份 + 套餐权益 + 使用环境许可”的职责。浏览器以 `localStorage` 中的随机 UUID 标识环境，它不能证明物理设备身份。v0.9 允许在仍保持单活环境的前提下自助重新激活，但没有多孩子、多监护人或多环境同时使用模型。
...
1. 管理员选择套餐并创建访问码。
2. 家长在首页输入访问码；首次使用时原子激活浏览器生成的环境 ID，不计入重新激活次数。
3. 如果访问码已在另一环境激活，首页展示重新激活确认；成功后原环境的活跃会话令牌失效，当前环境重新验证并进入监督。
4. 后端校验访问码状态、隐私确认、总剩余额度和使用环境，创建会话与会话令牌；会话创建后开始计时。
5. 监督页请求摄像头权限，用户确认摆放和声音可用。
6. 前端按套餐下限和学习时长动态安排识别：
   - 学习开始 15 分钟内期望 15 秒一次；
   - 15–60 分钟期望 30 秒一次；
   - 60–120 分钟期望 60 秒一次；
   - 120 分钟后期望 90 秒一次；
   - 最终频率不得高于套餐允许的最小间隔。
7. 状态异常时提高观察频率；稳定学习时适当降低频率。
8. 离座或无法判断达到条件后播放本地语音/蜂鸣，提醒冷却会随近期状态动态调整。
9. 用户可对最新识别结果手动标记。
10. 结束后服务端按数据库时间和真实记录结算，在数据库事务内结束会话并原子扣减访问码时长，生成报告访问令牌。
11. 报告可刷新、打印或保存为 PDF；首页本地保存最近报告入口。
...
- 访问码验证、隐私确认、首次环境激活、自助重新激活、永久有效总额度和生命周期状态。
...
- 习惯趋势使用最近最多 7 次可信监督计算，至少 3 次才形成方向判断；可信监督需不少于 10 分钟、至少 5 条真实识别记录、数据覆盖率不低于 50%，且不包含 Mock 记录。
...
| `access_codes` | 权益、当前使用环境、状态、额度 | 一对多会话 |
...
| `suspicious_logs` | 无效码、新环境、限流等风险 | 可关联访问码 |
| `admin_actions` | 后台权益操作审计 | 可关联访问码 |
| `device_rebind_configs` | 滚动窗口、次数上限和最小成功间隔配置 | 全局单行配置 |
| `device_rebind_logs` | 成功/失败重新激活、管理员重置与幂等结果 | 关联访问码 |
...
| `POST /api/device/rebind` | 服务端原子执行使用环境重新激活 | 访问码凭证 + 限流 + 幂等请求 ID |
...
| `GET/POST /api/admin/rebind-config` | 查看或更新滚动窗口、上限和最小间隔 | 后台密码 |
| `GET/POST /api/session/cleanup` | 结算超时会话 | 当前无鉴权 |
| `GET /api/cron/keep-alive` | 每日执行一次 Supabase 极轻量只读保活 | Vercel `CRON_SECRET` Bearer 鉴权 |
...
- 访问码首次使用时激活浏览器 `localStorage` 中的匿名环境 ID；它不是物理设备指纹。
- 真正重新激活由数据库锁定访问码后一次完成环境更新、滚动窗口计数、历史和风险标记；同一幂等请求复用第一次结果。
- 成功重新激活会轮换未结束会话的令牌，使原环境不能继续调用监督接口；原页面收到失效响应后停止摄像头、分析和心跳并返回首页。
- 重新激活不修改访问码总时长、已用时长、会话时长或报告；首次、同环境、取消、失败、幂等回放和管理员重置不计入用户次数。
- 核心监督接口校验访问码 ID、会话 ID 和高熵会话令牌。
- 会话结束后清空会话令牌；报告使用独立只读令牌。
- 数据库启用 RLS 且不给匿名浏览器开放策略。
- 新使用环境、无效访问码、黑名单、额度不足和限流会记录风险。
- 不做人脸识别，仓库中没有视频保存逻辑。
...
- 极端并发创建两个会话时，数据库还没有唯一活跃会话约束；重新激活能使原令牌失效，原子结算可防止总扣减超过额度，但同环境并发创建会话仍可能短暂同时运行。
- 超时清理和客户端错误接口没有专用鉴权/签名。
- 报告令牌放在 URL 中，可能进入浏览器历史或访问日志。
- 图片在 Qwen 模式下会发送给第三方模型；当前页面确认不是完整隐私政策、法务评估或可撤回的隐私中心。
- Next.js 14 当前生产依赖审计仍有 2 个高风险项；项目负责人已于 2026-08-04 接受 v0.9 本次发布风险，但不是永久豁免。自动修复需要升级到 Next.js 16，后续单独进行安全升级和完整回归。
- Known Limitation：真实 Qwen 上游已开始处理后发生 Abort 时，无法实机证明上游连接立即终止；旧响应不会影响识别记录、提醒、监督状态、报告、计时、套餐权益或数据库一致性，极端情况下可能多一次模型调用费用。项目负责人已接受其为非发布阻断项。
...
- 已增加权益、趋势样本和使用环境重新绑定规则/数据库并发契约测试及 GitHub Actions。
- migration 18 已在本机临时 PostgreSQL 18.4 和独立托管 Supabase PostgreSQL 17.6 上，以 19 个脱敏访问码完成迁移、只读验证、快照对比、RLS/函数权限、幂等、双连接并发和强制回滚测试；这不是生产真实 19 码快照，也不能代替独立测试部署和真机验收。
- 托管验证发现并修复 Supabase 默认向 `anon`、`authenticated` 授予新函数执行权的问题；当前 migration 与完整 schema 显式撤销浏览器角色，仅授予 `service_role`。
- 生产代码和界面版本基线为 `0.9.0 / 体验版 v0.9`；`0.9.1 / 体验版 v0.9.1` 只增加每日只读保活，已通过 Preview 验证并获 Production 发布确认。
- 生产 Supabase 已完成本轮迁移：19 个访问码到期时间清空、报告等级统一、隐私字段与原子结算函数均通过 SQL 校验。
- Vercel 生产部署和自定义域名已验收；真实手机已确认访问、摄像头、真实 `qwen3-vl-flash`、重新绑定、恢复监督、原子结算、手动结束和报告生成，最终生产烟雾测试已通过。额度耗尽和更长时间异常退出仍需在后续真实使用中持续观察。
- v0.9 已按显式事务 migration 18 → verify 18 → migration 19 → verify 19 → 一致性检查 → Deploy → 最终真机烟雾测试执行并通过生产验收。
- 监督请求隔离 migration 19 已在独立托管 Supabase 执行并验证，客户端取消和数据库旧令牌写入门禁均通过。Qwen 上游连接是否在已经开始处理后立即响应 Abort 作为已接受 Known Limitation 保留。
...
| 已接受发布风险 | Next.js 14 生产依赖审计有 2 个高风险项 | 安全与长期维护风险；不再阻断 v0.9，但不是永久豁免 | 单独升级 Next.js 16，在隔离环境完成全链路回归；升级前不扩展未评估攻击面 |
| P1 | 极端并发开始会话仍可能短暂超用 | 原子结算不超扣，但使用控制不完全 | 后续增加数据库级单活会话约束/开始会话 RPC |
| P2 | 字段/文案仍保留日额度和旧状态历史语义 | 增加维护和数据解释成本 | 做兼容迁移并统一命名 |
| P2 | 首页报告历史只保存在当前浏览器 | 重新激活后当前环境看不到原环境的本地入口 | 后续账户体系或可恢复的家庭身份 |
| P2 | 浏览器 UUID 可被清理或伪造 | 同一物理设备可能被视为新环境，不能作为强身份 | 通过滚动次数、最小间隔和异常标记控风险，规模化后再评估强身份 |
| P2 | 单个监督页超过 2,000 行 | 迭代容易相互影响 | 按摄像头、识别、提醒、结算拆分 |

## 13. 下一阶段建议顺序

1. **执行并观察 v0.9.1 小版本发布**：Preview 已确认每日保活接口鉴权、只读查询、失败路径和零业务写入；Production 发布已获负责人确认。
2. **建立真实基线**：确认线上模型，导出最近 7/30 天的使用、成功率、识别质量、提醒效果和成本。
