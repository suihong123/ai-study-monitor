...
> 基线日期：2026-07-27
> 生产代码：`a2bb6a6`
> 实施状态：v0.7 已部署至 `www.aistudyguard.top`；v0.8 监督与报告信息层级优化已在本地完成，待发布
...
- `package.json` 和界面版本已统一为 `0.8.0 / 体验版 v0.8`；生产环境仍为 v0.7，待本轮发布后切换。
- 生产 Supabase 已完成本轮迁移：19 个访问码到期时间清空、报告等级统一、隐私字段与原子结算函数均通过 SQL 校验。
- Vercel 生产部署和自定义域名首页/后台入口已验收；真实手机摄像头、Qwen、重复结算和异常退出仍需用测试访问码完成端到端观察。
