...
> 基线日期：2026-08-04
> 生产代码：`98088ff`（v0.8）
> 实施状态：v0.9 使用环境重新绑定及监督请求隔离已完成功能验收，5 个 P1 发布门禁已关闭，Release Candidate 待提交审核；尚未执行生产迁移、推送或部署
...
- v0.9 开发分支将设备绑定修正为浏览器“使用环境”激活：默认任意连续 15 天最多 10 次免费重新激活、两次成功至少间隔 60 秒，不扣监督时长；生产仍为 v0.8，不能视为已上线事实。
...
- 开发代码和界面版本为 `0.9.0 / 体验版 v0.9`；生产环境仍是 `0.8.0 / 体验版 v0.8`。
- 生产 Supabase 已完成本轮迁移：19 个访问码到期时间清空、报告等级统一、隐私字段与原子结算函数均通过 SQL 校验。
- Vercel 生产部署和自定义域名已验收；真实手机已确认访问、摄像头、识别、手动结束和报告生成，正式趋势报告也已用历史数据验收。当前视觉模式、实际额度扣减、重复结算、额度耗尽和异常退出仍待继续核对。
- v0.9 独立测试部署、测试密钥轮换、真实 Qwen 和项目负责人确认的核心真机验收已完成；生产按显式事务 migration 18 → verify 18 → migration 19 → verify 19 → 一致性检查 → Deploy → 验收执行，当前尚未上线。
...
1. **审核并提交 v0.9 Release Candidate**：5 个 P1 发布门禁已关闭；完成候选提交审核后，再进入生产 Migration → Deploy → 验收流程。
