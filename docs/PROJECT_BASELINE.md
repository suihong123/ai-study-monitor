...
> 基线日期：2026-07-27
> 生产代码：`b6bde4c`
> 实施状态：v0.8 已部署至 `www.aistudyguard.top`，进入真实使用观察
...
- 已增加权益和趋势样本规则测试及 GitHub Actions；本地类型检查、8 项测试、Lint 和生产构建均通过。
- `package.json`、界面和生产环境版本已统一为 `0.8.0 / 体验版 v0.8`。
...
1. **完成真实端到端验收**：用专用测试访问码覆盖手机摄像头、Qwen、重复结算、额度耗尽和异常退出。
