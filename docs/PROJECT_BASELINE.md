...
> 基线日期：2026-07-29
> 生产代码：`98088ff`（v0.8）
> 实施状态：v0.9 访问码自助换绑已完成开发验证，尚未执行生产迁移与部署
...
- v0.9 开发分支已实现单设备自助换绑：3 次免费换绑，之后按可配置分钟兑换，并带可配置冷却、数据库幂等和换绑历史；生产仍为 v0.8，不能视为已上线事实。
...
当前没有账户体系，因此一个访问码基本承担了“客户身份 + 套餐权益 + 设备许可”的职责。v0.9 允许在仍保持单设备的前提下自助更换绑定设备，但没有多孩子、多监护人或多设备同时使用模型。
...
1. 管理员选择套餐并创建访问码。
2. 家长在首页输入访问码；首次使用时原子绑定浏览器生成的设备 ID，不消耗免费换绑次数。
3. 如果访问码已绑定另一设备，首页展示换绑确认；成功后旧设备的活跃会话令牌失效，新设备重新验证并进入监督。
4. 后端校验访问码状态、隐私确认、总剩余额度和设备，创建会话与会话令牌；会话创建后开始计时。
...
- 访问码验证、隐私确认、首次设备绑定、自助换绑、永久有效总额度和生命周期状态。
...
| `device_rebind_configs` | 换绑扣时长和冷却配置 | 全局单行配置 |
| `device_rebind_logs` | 成功/失败换绑历史与幂等结果 | 关联访问码 |
...
| `POST /api/device/rebind` | 服务端原子执行设备换绑 | 访问码凭证 + 限流 + 幂等请求 ID |
...
| `GET/POST /api/admin/rebind-config` | 查看或更新换绑成本与冷却 | 后台密码 |
...
- 访问码首次使用绑定匿名设备 ID。
- 真正换绑由数据库锁定访问码后一次完成设备、免费次数/监督时长、历史和风险日志更新；同一幂等请求复用第一次结果。
- 成功换绑会轮换未结束会话的令牌，使旧设备不能继续调用监督接口。
...
- 极端并发创建两个会话时，数据库还没有唯一活跃会话约束；换绑能使旧令牌失效，原子结算可防止总扣减超过额度，但同设备并发创建会话仍可能短暂同时运行。
...
- 已增加权益、趋势样本和设备换绑规则/数据库并发契约测试及 GitHub Actions。
...
- v0.9 必须先执行 `migration_2026_18_device_rebind_mvp.sql`，再部署应用并用两台真实设备完成换绑验收；当前尚未上线。
...
| P2 | 首页报告历史只保存在当前浏览器 | 自助换绑后新设备看不到旧设备本地入口 | 后续账户体系或可恢复的家庭身份 |
| P2 | 单个监督页超过 2,000 行 | 迭代容易相互影响 | 按摄像头、识别、提醒、结算拆分 |

## 13. 下一阶段建议顺序

1. **安全上线并验收 v0.9**：先迁移、再部署，用专用码覆盖手机↔平板免费换绑、冷却、时长兑换和旧设备令牌失效。
